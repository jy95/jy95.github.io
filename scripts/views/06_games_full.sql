SELECT
    ...
    CASE
        WHEN schedule.id IS NULL THEN 'unscheduled'
        WHEN DATE('now') <= schedule."availableAt" THEN 'future'
        WHEN schedule."endAt" IS NOT NULL
             AND DATE('now') > schedule."endAt" THEN 'past'
        ELSE 'present'
...
LEFT JOIN games_schedules AS schedule
    ON schedule.id = g.id
