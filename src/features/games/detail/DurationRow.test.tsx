...
import PrettyDuration from './rows/DurationRow';

const gameWithDuration = (duration: string): ComponentProps<typeof PrettyDuration>['game'] =>
    ({ duration } as ComponentProps<typeof PrettyDuration>['game']);

describe('PrettyDuration', () => {
    it('renders hours and minutes when both are present', () => {
        render(<PrettyDuration game={gameWithDuration('02:15:00')} />);
        expect(screen.getByText('2 hours 15 minutes')).toBeInTheDocument();
    });

    it('renders only minutes when hours is 0', () => {
        render(<PrettyDuration game={gameWithDuration('00:45:00')} />);
        expect(screen.getByText('45 minutes')).toBeInTheDocument();
    });

    it('renders only hours when minutes is 0', () => {
        render(<PrettyDuration game={gameWithDuration('03:00:00')} />);
        expect(screen.getByText('3 hours')).toBeInTheDocument();
    });

    it('falls back to seconds only when both hours and minutes are 0', () => {
        render(<PrettyDuration game={gameWithDuration('00:00:30')} />);
        expect(screen.getByText('30 seconds')).toBeInTheDocument();
    });

    it('renders nothing when the whole duration is zero', () => {
        const { container } = render(<PrettyDuration game={gameWithDuration('00:00:00')} />);
        expect(container.textContent).toBe('');
    });

    it('defaults missing minutes/seconds segments to 0 when duration is a partial string', () => {
        render(<PrettyDuration game={gameWithDuration('05')} />);
        expect(screen.getByText('5 hours')).toBeInTheDocument();
    });

    it('never shows seconds when hours or minutes are present, even if seconds are non-zero', () => {
        render(<PrettyDuration game={gameWithDuration('01:00:45')} />);
    ...
    it('renders a different value on re-render with a new duration prop', () => {
        const { rerender } = render(<PrettyDuration game={gameWithDuration('01:00:00')} />);
        expect(screen.getByText('1 hours')).toBeInTheDocument();

        rerender(<PrettyDuration game={gameWithDuration('00:30:00')} />);
